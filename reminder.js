...
exports.run = (client, message, args) => {
  ...
  if (validUnlocks.includes(time)) {
    ...
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.sendMessage(`${message.author} **⌚⌚⌚YOU HAVE SET A REMINDER FOR __${ms(ms(time), { long:true })}__!!!⌚⌚⌚**`).then(() => {

        client.lockit[message.channel.id] = setTimeout(() => {
          message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: true
          }).then(message.channel.sendMessage(`${message.author}**⏰⏰⏰YOUR REMINDER HAS __ENDED__!!!⏰⏰⏰**`)).catch(console.error);
          delete client.lockit[message.channel.id];
        }, ms(time));
